%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [bestx,bestf] = sceua_n(x0,bl,bu,maxn,kstop,pcento,peps,ngs,iseed,iniflg, ...
                                                                       F1,f10,Xw,p,Mw,V,c)

% This is the subroutine implementing the SCE algorithm, 
% written by Q.Duan, 9/2004
% Adapted by B.Krieger, 3/2020
%
% Definition:
...
icall=0;
for i=1:npt
    xf(i) = EVM_Inner(x(i,:),F1,f10,Xw,p,Mw,V,c);
    icall = icall + 1;
end
f0=xf(1);
...
while icall<maxn && gnrng>peps && criter_change>pcento
    ...
    for igs = 1: ngs
        ...
        for loop=1:nspl;
            ...
            [snew,fnew,icall]=cceua_n(s,sf,bl,bu,icall,maxn,F1,f10,Xw,p,Mw,V,c);
